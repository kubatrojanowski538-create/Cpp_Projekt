...
def evaluate(model, loader, loss_fn, device):
    ...
    with torch.no_grad():
        for x, y in loader:
            ...
            brake_pred = preds[:, brake_index]
            brake_true = y[:, brake_index]

            brake_tp += ((brake_pred == 1.0) & (brake_true == 1.0)).sum().item()
            brake_fp += ((brake_pred == 1.0) & (brake_true == 0.0)).sum().item()
            brake_fn += ((brake_pred == 0.0) & (brake_true == 1.0)).sum().item()
    ...
    eps = 1e-8
    brake_precision = brake_tp / (brake_tp + brake_fp + eps)
    brake_recall = brake_tp / (brake_tp + brake_fn + eps)
    brake_f1 = 2.0 * brake_precision * brake_recall / (brake_precision + brake_recall + eps)

    brake_metrics = {
        "precision": brake_precision,
        "recall": brake_recall,
        "f1": brake_f1,
    }

    return avg_loss, exact_acc, per_button_acc.cpu().numpy(), brake_metrics
...
def save_training_outputs(history, pos_weight, best_epoch, best_val_loss):
    ...
    # 5. Brake precision and F1 over epochs
    plt.figure(figsize=(10, 6))
    plt.plot(
        history_df["epoch"],
        history_df["brake_precision"] * 100.0,
        label="brake_precision",
    )
    plt.plot(
        history_df["epoch"],
        history_df["brake_f1"] * 100.0,
        label="brake_f1",
    )
    plt.axvline(best_epoch, linestyle="--", label=f"best epoch = {best_epoch}")
    plt.xlabel("Epoch")
    plt.ylabel("Metric [%]")
    plt.ylim(0, 100)
    plt.title("Validation brake precision and F1")
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(RESULTS_DIR / "brake_precision_f1_curve.png", dpi=160)
    plt.close()

    # 6. Brake precision and F1 at the best epoch
    brake_best_values = [
        best_row["brake_precision"] * 100.0,
        best_row["brake_f1"] * 100.0,
    ]

    plt.figure(figsize=(8, 6))
    plt.bar(["brake_precision", "brake_f1"], brake_best_values)
    plt.ylabel("Metric [%]")
    plt.ylim(0, 100)
    plt.title("Brake precision and F1 at best validation loss")
    plt.grid(True, axis="y", alpha=0.3)
    plt.tight_layout()
    plt.savefig(RESULTS_DIR / "best_epoch_brake_precision_f1.png", dpi=160)
    plt.close()

    # 7. pos_weight bar plot
    plt.figure(figsize=(9, 6))
    ...
    # 9. Separate summary focused on the brake output
    ...
    axes[0].set_title("Brake precision and F1")
    ...
    axes[1].bar(["precision", "F1"], brake_best_values)
    axes[1].set_title("Best epoch brake metrics")
    axes[1].set_ylabel("Metric [%]")
    axes[1].set_ylim(0, 100)
    axes[1].grid(True, axis="y", alpha=0.3)

    fig.suptitle("Brake validation metrics", fontsize=14)
    fig.tight_layout()
    fig.savefig(RESULTS_DIR / "brake_metrics_summary.png", dpi=160)
    plt.close(fig)
    ...
    with report_path.open("w", encoding="utf-8") as file:
        ...
        file.write(f"- `brake_precision`: `{best_row['brake_precision'] * 100.0:.2f}%`\n")
        file.write(f"- `brake_recall`: `{best_row['brake_recall'] * 100.0:.2f}%`\n")
        file.write(f"- `brake_f1`: `{best_row['brake_f1'] * 100.0:.2f}%`\n")
        file.write("\n## Generated files\n\n")
        ...
        file.write("- `brake_precision_f1_curve.png`\n")
        file.write("- `best_epoch_brake_precision_f1.png`\n")
        file.write("- `brake_metrics_summary.png`\n")
...
def train():
    ...
    for epoch in range(1, epochs + 1):
        ...
        val_loss, exact_acc, per_button_acc, brake_metrics = evaluate(
            model,
            val_loader,
            loss_fn,
            device
        ...
        row = {
            ...
            "val_loss": val_loss,
            "exact_acc": exact_acc,
            "brake_precision": brake_metrics["precision"],
            "brake_recall": brake_metrics["recall"],
            "brake_f1": brake_metrics["f1"],
        }
        ...
        if epoch == 1 or epoch % 5 == 0:
            ...
            print(
                f"  brake_precision={brake_metrics['precision'] * 100:.2f}% | "
                f"brake_f1={brake_metrics['f1'] * 100:.2f}%"
            )
